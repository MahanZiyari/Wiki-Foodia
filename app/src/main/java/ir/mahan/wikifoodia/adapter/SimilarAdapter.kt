...
import ir.mahan.wikifoodia.databinding.ItemSimilarBinding
import ir.mahan.wikifoodia.models.detail.ResponseSimilar.ResponseSimilarItem
    ...
    inner class ViewHolder : RecyclerView.ViewHolder(binding.root) {
        ...
        fun bind(item: ResponseSimilarItem) {
            binding.apply {
                ...
                //Image
                val image =
                    "${Constants.BASE_URL_IMAGE_RECIPE}${item.id}-${Constants.NEW_IMAGE_SIZE}"
                coverImg.load(image) {
                    crossfade(true)
                    crossfade(800)
